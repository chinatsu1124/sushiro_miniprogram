...
Component({
  ...
  methods: {
      ...
      try {
        ...
        if (response.success && response.regions && response.regions.length > 0) {
          ...
          if (this.data.selectedRegion) {
            app.setSelectedRegion(this.data.selectedRegion)
          }
    ...
    async loadData() {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          // 根据错误类型显示不同的提示信息
          this.handleDataError(response.message || '数据加载失败')
        }
      } catch (error: any) {
        this.handleDataError(`数据加载失败: ${error.message}`)
      } finally {
        this.setData({ loading: false })
      }
    },
    ...
    // 处理数据加载错误
    handleDataError(errorMessage: string) {
